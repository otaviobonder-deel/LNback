...
routes.get('/walletbalance', async (req, res) => {
    try {
        const { chain_balance: chainBalance } = await lnService.getChainBalance({lnd})
...
routes.get('/chaingraph', async (req, res) => {
    try {
        ...
        return res.json({ nodes: mappedNodes, links: channels });
    } catch (e) {
...
routes.get('/nodeinfo/:id', async (req, res) => {
    try {
        const {channels, capacity} = await lnService.getNode({lnd, public_key: req.params.id})
        const nodeChannels = []

        for (const channel of channels) {
            let otherNode = channel.policies.find(c => c.public_key !== req.params.id)
            nodeChannels.push({
